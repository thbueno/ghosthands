...
// ---- TrailManager: ping-pong simulation ----
...
class TrailManager {
  ...
  update(mouseX: number, mouseY: number, mouseActive: boolean) {
    ...
    // Render decay shader to write target
...
function TrailSystem() {
  ...
  const { geometry, material } = useMemo(() => {
    ...
    // Plane geometry for each instance
    const baseGeo = new THREE.PlaneGeometry(0.8, 0.8);

    // Instanced geometry with custom attributes
    ...
    instancedGeo.setAttribute(
      'instancePosition',
      new THREE.InstancedBufferAttribute(attrs.positions, 3)
    );
    instancedGeo.setAttribute(
      'instanceIndex',
      new THREE.InstancedBufferAttribute(attrs.indices, 1)
    );
    instancedGeo.setAttribute(
      'instanceProgress',
      new THREE.InstancedBufferAttribute(attrs.progress, 1)
    );
