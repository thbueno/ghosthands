...
- **Custom social icons in `ProfileHeader`**: Replaced standard Lucide icons (`Linkedin`, `Mail`) in the profile header with custom SVG icons. The LinkedIn and GitHub icons are styled as matching squircles using assets from `public/images/social-icons` (size set to `28`). The WhatsApp and Email icons are styled as clean outlines without solid background boxes (size set to `20`), using `currentColor` so they transition dynamically to `text-secondary` on hover. Their sizes are adjusted to match the height of the internal glyphs of the other icons perfectly.
...
- **GitHub social link in `ProfileHeader`**: Added a GitHub link with a custom squircle GitHub icon (`GitHubIcon`) to the social links list in the profile header.
